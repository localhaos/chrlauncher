...
static VOID _app_copy_legacy_config_if_needed (
...
{
	WCHAR current_section[128];
	WCHAR base_section[128];
	WCHAR probe_buffer[4] = {0};
	LPWSTR section_buffer;
	DWORD section_length;
	const DWORD section_buffer_chars = 65536;

	if (_r_obj_isstringempty (config_path) || !_r_fs_exists (&config_path->sr))
		return;

	_r_str_copy (current_section, RTL_NUMBER_OF (current_section), _r_app_getnameshort ());
	_r_str_copy (base_section, RTL_NUMBER_OF (base_section), APP_NAME_SHORT);

	if (lstrcmpiW (current_section, base_section) == 0)
		return;

	if (GetPrivateProfileSectionW (current_section, probe_buffer, RTL_NUMBER_OF (probe_buffer), config_path->buffer) > 0)
		return;

	section_buffer = _r_mem_allocate (section_buffer_chars * sizeof (WCHAR));

	if (!section_buffer)
		return;

	section_buffer[0] = UNICODE_NULL;
	section_buffer[1] = UNICODE_NULL;

	section_length = GetPrivateProfileSectionW (base_section, section_buffer, section_buffer_chars, config_path->buffer);

	if (section_length)
		WritePrivateProfileSectionW (current_section, section_buffer, config_path->buffer);

	_r_mem_free (section_buffer);
}
...
VOID _app_initialize_addons_config ()
{
	...
	_app_ensure_config_section_alias (config_path);
