...
)
{
	...
	_r_obj_initializestringref (&empty_string, (LPWSTR)string);
...
)
{
	...
	else
	{
		if (is_directory)
		{
			status = _r_fs_deletedirectory (path, TRUE);
		}
		else
		{
...
)
{
	...
	status = _r_path_getfullpath (string->buffer, &user_data_dir);
...
BOOLEAN _app_apply_last_profile_taskbar_hook (
	_Inout_ PBROWSER_INFORMATION pbi
)
{
	...
	if (_r_obj_isstringempty (profile_argument))
	{
		if (profile_argument)
	...
	_app_append_browser_arguments (&pbi->args_str, profile_argument);
...
BOOLEAN _app_is_repairable_taskbar_target (
	_In_ PBROWSER_INFORMATION pbi,
	_In_ LPCWSTR launcher_path,
	_In_ LPCWSTR target_path
)
{
	R_STRINGREF target_sr;
	LPCWSTR binary_name;
	LPCWSTR target_name;
	ULONG_PTR next_char_index;

	if (_r_str_isempty (target_path))
		return FALSE;

	if (_wcsicmp (target_path, launcher_path) == 0)
		return TRUE;

	if (!_r_obj_isstringempty (pbi->binary_path) && _wcsicmp (target_path, pbi->binary_path->buffer) == 0)
		return TRUE;

	if (_r_obj_isstringempty (pbi->binary_dir) || _r_obj_isstringempty (pbi->binary_path))
		return FALSE;

	target_name = PathFindFileNameW (target_path);
	binary_name = PathFindFileNameW (pbi->binary_path->buffer);

	if (_r_str_isempty (target_name) || _r_str_isempty (binary_name))
		return FALSE;

	if (_wcsicmp (target_name, binary_name) != 0)
		return FALSE;

	_r_obj_initializestringref (&target_sr, (LPWSTR)target_path);

	if (target_sr.length <= pbi->binary_dir->length || !_r_str_isstartswith (&target_sr, &pbi->binary_dir->sr, TRUE))
		return FALSE;

	next_char_index = pbi->binary_dir->length / sizeof (WCHAR);

	return target_path[next_char_index] == OBJ_NAME_PATH_SEPARATOR || target_path[next_char_index] == OBJ_NAME_ALTPATH_SEPARATOR;
}
	...
	do
	{
		...
		if (SUCCEEDED (hresult))
		{
			...
			if (SUCCEEDED (hresult))
			{
				...
				if (SUCCEEDED (hresult))
				{
					arguments[0] = UNICODE_NULL;
					target_path[0] = UNICODE_NULL;
					working_directory[0] = UNICODE_NULL;

					if (SUCCEEDED (link->lpVtbl->GetPath (link, target_path, RTL_NUMBER_OF (target_path), NULL, SLGP_RAWPATH)))
					{
						if (_app_is_repairable_taskbar_target (pbi, launcher_path, target_path))
						{
							is_modified = FALSE;

							link->lpVtbl->GetArguments (link, arguments, RTL_NUMBER_OF (arguments));
							link->lpVtbl->GetWorkingDirectory (link, working_directory, RTL_NUMBER_OF (working_directory));
							...
							if (app_directory && _wcsicmp (working_directory, app_directory->buffer) != 0)
							{
...
)
{
	...
	PR_STRING close_tabs_right_arguments;
	...
	PR_STRING scrollable_tabs_features;
	...
	string = _app_resolve_configured_path (binary_dir, L".", FALSE);
	...
	_r_str_trimstring (&pbi->binary_dir->sr, &separator_sr, 0);
	...
	if (!_app_apply_last_profile_taskbar_hook (pbi))
	...
	*is_error_ptr = FALSE;
	...
	if (hashtable)
	{
		...
		else
		{
			...
			_app_config_setlong64 (L"ChromiumLastCheck", _r_unixtime_now ());
		}
...
#define kInputBufSize ((ULONG_PTR)1 << 18)
	...
	for (ULONG_PTR i = 0; i < db.NumFiles; i++)
	{
		...
		else
		{
			...
			if (sub_dir)
			{
				if (!_r_fs_isdirectory (&sub_dir->sr))
	...
	switch (msg)
	{
		...
		case WM_COMMAND:
		{
			...
			switch (ctrl_id)
			{
				...
				case IDM_TRAY_RUN:
				{
					_app_openbrowser (hwnd, &browser_info);
					break;
				}
				...
				case IDM_RUNATEND_CHK:
				{
					...
					_app_config_setboolean (L"ChromiumRunAtEnd", new_val);
