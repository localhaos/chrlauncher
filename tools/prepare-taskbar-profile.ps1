...
$text = $text.Replace(
    'if (pbi->is_hasurls)' + "`r`n" + "\t" + '{' + "`r`n" + "\t\t" + 'profile_argument = _app_get_profile_argument (pbi);',
    'if (pbi->is_hasurls || _app_config_getboolean (L"ChromiumUseLastProfileOnTaskbarLaunch", TRUE))' + "`r`n" + "\t" + '{' + "`r`n" + "\t\t" + 'profile_argument = _app_get_profile_argument (pbi);'
)
